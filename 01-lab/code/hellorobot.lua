...
MOVE_STEPS = 15 -- global variables, there are not constant, usually the convention is that if they are in capital letter we should not change them.
MAX_VELOCITY = 10
LIGHT_THRESHOLD = 1.5
...
function step()
	n_steps = n_steps + 1
	-- This is a way to move it randomly, BUT NOT THE BEST WAY. (try to think a better way)
	...
	light_front = robot.light[1].value + robot.light[24].value -- There is no a frontal light sensor, they are 24, so the 1 and the 24 are the more frontal ones (see the docs in the notes).
	log("robot.light_front = " .. light_front)
	ground = robot.motor_ground --just to save some chars... They are the ground sensors
